...
-- 2.c
SELECT B.BOOK_ID, B.TITLE, COUNT(C.COPY_ID) AS COPIES_COUNT FROM BOOK_TBL B
LEFT JOIN COPY_TBL C ON B.BOOK_ID = C.BOOK_ID
GROUP BY B.BOOK_ID, B.TITLE;
...
-- 3.b
SELECT I.INVOICE_ID, I.ISSUE_DATE, I.INVOICE_NUMBER, COUNT(C.COPY_ID) AS COPIES_COUNT, SUM(C.GROSS_PRICE) AS TOTAL_PRICE
FROM INVOICE_TBL I
...
-- 5.a
SELECT B.TITLE, COUNT(C.COPY_ID) AS COPIES_COUNT FROM BOOK_TBL B
LEFT JOIN COPY_TBL C ON B.BOOK_ID = C.BOOK_ID
WHERE TITLE = 'Mystery of the Ancients'
GROUP BY B.TITLE;

-- 5.b
SELECT P.PUBLISHER_NAME, COUNT(I.INVOICE_ID) AS INVOICES_COUNT FROM PUBLISHER_TBL P
...
-- 7.a
SELECT C.COPY_ID, COUNT(R.RENTAL_ID) AS BORROWED_COUNT FROM COPY_TBL C
LEFT JOIN RENTAL_TBL R ON C.COPY_ID = R.COPY_ID
GROUP BY C.COPY_ID;

-- 7.b
SELECT R.FIRST_NAME, R.LAST_NAME, COUNT(RE.RENTAL_ID) AS BORROWED_COUNT FROM READER_TBL R
LEFT JOIN RENTAL_TBL RE ON R.READER_ID = RE.READER_ID
GROUP BY R.FIRST_NAME, R.LAST_NAME;

-- 7.c
SELECT E.FIRST_NAME, E.LAST_NAME, COUNT(R.RENTAL_ID) AS LENT_COUNT FROM EMPLOYEE_TBL E
